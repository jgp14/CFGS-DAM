		...
		if (total == 0) {
			...
			try {
				...
				for (int i = 0; i < total; i++) {
					flujoSalida.writeUTF(fruteria[i].getCodigo());
					flujoSalida.writeUTF(fruteria[i].getNombre());
					flujoSalida.writeInt(fruteria[i].getCantidad());
					flujoSalida.writeFloat(fruteria[i].getPrecio());
				}
			} catch (IOException e) {
