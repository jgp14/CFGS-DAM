    ...
    public void addNota(View view) {
        if (db != null) {
            if (comprobarDatos()) {
                if (existeAlumno()) {
                    try {
                        db.execSQL("insert into Notas(NumMat, NomMod, Nota) values " +
                                "('" + etMatri.getText().toString() +
                                "', '" + etModulo.getText().toString() + "', '" +
                                etNota.getText().toString() + "');");
                        Toast.makeText(this, "Nota añadida", Toast.LENGTH_LONG).show();
                    } catch (SQLiteConstraintException e) {
                        Toast.makeText(this, "Ya se ha asignado una nota para ese módulo",
